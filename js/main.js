...
define([ 'jquery', 'gui', 'amplify', 'mousetrap' ], ($) => {
	...
	Mousetrap.bind('ctrl+shift+pageup', () => decrementWidgetVisible());
	Mousetrap.bind('ctrl+shift+pagedown', () => incrementWidgetVisible());
	 ...
	 *  @type {Array}
	 */
	wgtMap = [ 'conversion-widget', 'settings-widget', 'help-widget' ];
	/**
	...
	/**
	 *  Make the next widget visible with looping.
	 *
	 *  @method incrementWidgetVisible
	 *
	 *  @return {boolean}               true - Success
	 */
	incrementWidgetVisible = function () {

		let nextWidget = wgtMap.indexOf(wgtVisible) + 1;

		if (nextWidget >= wgtMap.length) {

			nextWidget = 0;

		}

		makeWidgetVisible(wgtMap[nextWidget]);

	};

	/**
	 *  Make the previous widget visible with looping.
	 *
	 *  @method decrementWidgetVisible
	 *
	 *  @return {boolean}               true - Success
	 */
	decrementWidgetVisible = function () {

		let nextWidget = wgtMap.indexOf(wgtVisible) - 1;

		if (nextWidget < 0) {

			nextWidget = wgtMap.length - 1;
